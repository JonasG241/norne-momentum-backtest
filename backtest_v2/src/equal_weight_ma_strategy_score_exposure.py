...
# MA-score exposure rule:
#  4 -> 100%,  3 -> 75%,  2 -> 50%,  1 -> 25%
#  0 -> 0%
# -1 -> 75%, -2 -> 50%, -3 -> 25%, -4 -> 0%
MA_SCORE_EXPOSURE_MAP = {
    4: 1.00,
    3: 0.75,
    2: 0.50,
    1: 0.25,
...
def ma_score_exposure_signal(data: dict[str, pd.DataFrame | pd.Series]) -> pd.DataFrame:
    """
    Converts the MA score into a long-only exposure factor using the table above.

    This is not binary. The value is a fraction between 0 and 1.
    The signal is shifted one day so today's position uses yesterday's score.
    """
    score = ma_score(data["price"], data["ma50"], data["ma100"], data["ma200"])
    exposure = score.replace(MA_SCORE_EXPOSURE_MAP).astype(float)
...
def main() -> tuple[dict[str, dict], pd.DataFrame, dict]:
    ...
    strategy_specs = {
        ...
        "MA_score": ma_score_exposure_signal(data),
    }
    ...
    diagnostics = run_all_diagnostics(
        ...
        data=data,
        top_n=20,
        strategy_names=["MA50_200_overlay", "MA50_200_selection"],
    )
...
if __name__ == "__main__":
    ...
    print("\nStrategy regression table. transaction_cost = ", TRANSACTION_COST)
...
def remove_top_n_contributing_stocks_strategy(
    result: dict,
    data: dict,
    n_remove: int = 3,
) -> tuple[dict, list[str]]:
    """
    Removes the top n stocks by total strategy contribution from an already-run
    selection strategy, then recomputes the strategy.

    For selection strategies, the remaining selected stocks are renormalized
    to sum to 1. If no selected stocks remain, the portfolio goes to cash.
    """
    target_original = result["target_weights"].fillna(0.0)
    rets = data["asset_returns"].reindex_like(target_original)
    rf = data["rf"].reindex(target_original.index)

    stock_contrib = (
        target_original * rets.fillna(0.0)
    ).sum().sort_values(ascending=False)

    removed_stocks = stock_contrib.head(n_remove).index.tolist()

    target_removed = target_original.copy()
    target_removed.loc[:, removed_stocks] = 0.0

    # Because this is a selection strategy, re-normalize remaining selected stocks.
    # If no stocks remain selected on a day, the strategy holds cash.
    denom = target_removed.sum(axis=1).replace(0.0, np.nan)
    target_removed = target_removed.div(denom, axis=0).fillna(0.0)

    strategy_returns_removed = portfolio_returns_with_tc(
        target_weights=target_removed,
        asset_returns=rets,
        rf=rf,
        fee=TRANSACTION_COST,
    )

    benchmark_target = data["benchmark_target"].reindex_like(target_removed)

    market_returns_removed = portfolio_returns_with_tc(
        target_weights=benchmark_target,
        asset_returns=rets,
        rf=rf,
        fee=0.0,
    ...
    eval_idx = strategy_returns_removed.index.intersection(market_returns_removed.index)

    eval_result = evaluate_strategy(
        strategy_returns=strategy_returns_removed.loc[eval_idx],
        market_returns=market_returns_removed.loc[eval_idx],
        rf=rf.loc[eval_idx],
    )

    eval_result["strategy_returns"] = strategy_returns_removed
    eval_result["market_returns"] = market_returns_removed
    eval_result["target_weights"] = target_removed
    eval_result["removed_stocks"] = removed_stocks
    eval_result["removed_total_contribution"] = stock_contrib.loc[removed_stocks].sum()

    return eval_result, removed_stocks


def make_ma50_200_removed_top3_table(
    original_result: dict,
    removed_result: dict,
) -> pd.DataFrame:
    """
    Prints a readable table for original MA50/200 selection and
    MA50/200 selection with top 3 contributing stocks removed.
    """
    rows = []

    for name, result in [
        ("MA50/200 selection", original_result),
        ("MA50/200 selection, top 3 stocks removed", removed_result),
    ]:
        rows.append(
            {
                "strategy": name,
                "cumulative_return": cumulative_simple_returns(
    ...
    print("\nMA50/200 selection: original vs top 3 stocks removed")
    print("=" * 100)
...
def plot_ma50_200_removed_top3(
    original_result: dict,
    removed_result: dict,
    equal_weight_osebx: pd.Series,
    market_weight_osebx: pd.Series,
    ...
    """
    Plots original MA50/200 selection, MA50/200 selection without top 3 stocks,
    equal-weight OSEBX and market-weight OSEBX.
    """
    original_returns = original_result["strategy_returns"].astype(float)
    removed_returns = removed_result["strategy_returns"].astype(float)

    idx = original_returns.index
    idx = idx.intersection(removed_returns.index)
    idx = idx.intersection(equal_weight_osebx.index)
    idx = idx.intersection(market_weight_osebx.index)

    chart = pd.concat(
        [
            cumulative_simple_returns(original_returns.reindex(idx)).rename(
                "MA50/200 selection"
            ),
            cumulative_simple_returns(removed_returns.reindex(idx)).rename(
                "MA50/200 selection, top 3 removed"
            ),
            cumulative_simple_returns(equal_weight_osebx.reindex(idx)).rename(
                "Equal-weight OSEBX"
            ),
            cumulative_simple_returns(market_weight_osebx.reindex(idx)).rename(
                "Market-weight OSEBX"
            ),
        ],
        axis=1,
    ...
    ax = chart.plot(figsize=(12, 6), linewidth=1.8)
    ax.set_title("MA50/200 selection: effect of removing top 3 contributing stocks")
    ax.set_ylabel("Cumulative return")
    ax.yaxis.set_major_formatter(plt.FuncFormatter(lambda value, _: f"{value:.0%}"))
    ax.grid(True, alpha=0.25)
    ax.legend(loc="best")
...
ma50_200_original = results["MA50_200_selection"]

ma50_200_removed_top3, removed_stocks = remove_top_n_contributing_stocks_strategy(
    result=ma50_200_original,
    data=data,
    n_remove=3,
)

print("\nRemoved top 3 contributing stocks:")
for stock in removed_stocks:
    print(stock)

ma50_200_removed_table = make_ma50_200_removed_top3_table(
    original_result=ma50_200_original,
    removed_result=ma50_200_removed_top3,
)

plot_ma50_200_removed_top3(
    original_result=ma50_200_original,
    removed_result=ma50_200_removed_top3,
    equal_weight_osebx=data["market_returns"],
    market_weight_osebx=market_weight_osebx,
